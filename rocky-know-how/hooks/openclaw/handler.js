...
`.trim();
  ...
  if (event.type === 'message' && event.action === 'received') {
    ...
    // === 注入相关经验到AI上下文 ===
    const isProblem = /错误|坏了|挂了|超时|失败|故障|502|500|404/.test(message);
    if (isProblem && message.length > 5) {
      injectExperience(scriptsDir, stateDir, message, event);
    }
 ...
 * 注入相关经验到AI上下文
 */
function injectExperience(scriptsDir, stateDir, message, event) {
  try {
    debugLog("INJECT_EXP: " + message.substring(0, 30));
    
    const expFile = join(stateDir, '.learnings', 'experiences.json');
    if (!existsSync(expFile)) return;
...
for exp_id, exp in entries.items():
    ...
    if score > 0:
        candidates.append({'id': exp_id, 'score': score, 'problem': exp.get('problem', ''), 'solution': exp.get('solution', ''), 'script': exp.get('script', '')})

candidates.sort(key=lambda x: x['score'], reverse=True)
for c in candidates[:3]:
    print(c['id'] + '|' + str(c['score']) + '|' + c['problem'] + '|' + c['solution'])
" 2>/dev/null || echo ""`,
      {encoding: 'utf8', timeout: 10000}
    ).trim();
    
    if (!searchResult) {
      debugLog("INJECT_EXP: no match");
      return;
    }
    
    // 构建经验参考内容
    const lines = searchResult.split('\n');
    let expContent = '# 相关经验参考\n\n';
    
    for (const line of lines) {
      const parts = line.split('|');
      if (parts.length >= 4) {
        const [expId, score, problem, solution] = parts;
        expContent += `## ${expId} (匹配度: ${score})\n`;
        expContent += `- 问题: ${problem}\n`;
        expContent += `- 解决方案: ${solution}\n\n`;
      }
    }
    ...
    if (Array.isArray(event.context.bootstrapFiles)) {
      event.context.bootstrapFiles.push({
        path: 'EXPERIENCE_REFERENCE.md',
        content: expContent,
        virtual: true,
      });
      debugLog("INJECT_EXP: injected " + lines.length + " experiences");
    }
  } catch (e) {
    debugLog("INJECT_EXP_ERROR: " + (e.message || '').substring(0, 30));
  }
}
